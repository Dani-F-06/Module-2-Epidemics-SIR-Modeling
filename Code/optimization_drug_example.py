...
def newtons_method(f, x0, tol=1e-6, max_iter=2000):
    x = x0
    for i in range(max_iter):
        grad = gradient(f, x)
...
# %% Combined function + optimization + lambda sweep

# Num 1 combined effect function (Metformin + Lisinopril + Escitalopram)
def combined_effect(x):
    return metformin(x) + lisinopril(x) + escitalopram(x)

# Plot the combined function on same graph as the 3 drugs
plt.figure(figsize=(10, 6))
plt.plot(x, metformin(x), label='Metformin', color='blue')
plt.plot(x, lisinopril(x), label='Lisinopril', color='orange')
plt.plot(x, escitalopram(x), label='Escitalopram', color='green')
plt.plot(x, combined_effect(x), label='Combined (M+L+E)', linestyle='--', linewidth=3, color='red') #adds combined function to the plot
#plot labels
plt.title('Drug Efficacy vs Dosage (Including Combined Effect)')
plt.xlabel('Dosage (mg)')
plt.ylabel('Net Effect')
plt.legend()
plt.show()
...
# Num 2 Apply steepest ascent and Newton’s method to the combined function
opt_dose_comb_sa, opt_effect_comb_sa = steepest_ascent(combined_effect, x0=1.0)
print(f"Steepest Ascent - Optimal Combined Dose: {opt_dose_comb_sa:.2f} mg")
print(f"Steepest Ascent - Optimal Combined Effect: {opt_effect_comb_sa*100:.2f}%")

opt_dose_comb_nm, opt_effect_comb_nm = newtons_method(combined_effect, x0=1.0)
print(f"Newton's Method - Optimal Combined Dose: {opt_dose_comb_nm:.2f} mg")
print(f"Newton's Method - Optimal Combined Effect: {opt_effect_comb_nm*100:.2f}%")


# Num 3 Choose a drug to modify and tune its lambda to try to match the combined optimum dose from Newton's method.
# Tune Metformin 

target_combined_dose = opt_dose_comb_nm  # use Newton combined optimum as the target

lambda_values = np.linspace(0.0, 2.0, 81)  # test lambda values (0 to 2)
best_lambda = None
best_dose = None
best_effect = None
best_diff = float('inf')
...
for lam in lambda_values:
    def metformin_tuned(x, lam=lam):
        efficacy = 0.8 * np.exp(-0.1*(x-5)**2)
        toxicity = 0.2 * x**2 / 100
        return efficacy - lam * toxicity

    # Re-calculate metformin optimum 
    dose_nm, effect_nm = newtons_method(metformin_tuned, x0=1.0)

    diff = abs(dose_nm - target_combined_dose)
    if diff < best_diff:
        best_diff = diff
        best_lambda = lam
        best_dose = dose_nm
        best_effect = effect_nm

print("\n--- Lambda sweep (tuning Metformin) ---")
print(f"Target combined optimal dose (Newton): {target_combined_dose:.2f} mg")
print(f"Best Metformin lambda: {best_lambda:.3f}")
print(f"Metformin optimal dose at best lambda: {best_dose:.2f} mg")
print(f"Difference from target: {best_diff:.2f} mg")
print(f"Metformin effect at best lambda: {best_effect*100:.2f}%")
